...
GegnerSmallWespe::GegnerSmallWespe(int Wert1, int Wert2, bool Light) {
    ...
    Value2 = static_cast<int>(pAim->ypos) - 100 + GetRandom(50);   // setzen mit etwas Variation
    ChangeLight = Light;
    Destroyable = true;
}
    ...
    switch (Handlung) {
        case GEGNER::LAUFEN:  // Warten bis der Spieler nahe genug rankommt
        {
            if (PlayerAbstand() < 500) {
                ...
                Value2 = static_cast<int>(pAim->ypos) + 40 - 50 + GetRandom(30);  // setzen mit etwas Variation
