...
void GegnerWasserMine::GegnerExplode() {
    ...
    for (int i = 0; i < 50; i++)
        PartikelSystem.PushPartikel(xPos - 5 + random(40), yPos - 5 + random(60), WATERFLUSH_HIGH);

    for (int i = 0; i < 25; i++)
        PartikelSystem.PushPartikel(xPos - 5 + random(40), yPos - 50 + random(110), SPIDERSPLITTER);
