...
void GegnerShootButton::DoKI() {
    ...
    if (Energy < 100.0f) {
        ...
        if (SmokeCount <= 0.0f) {
            SmokeCount = 0.2f;
            Projectiles.PushProjectile(pPlattForm->xPos + 23.0f + static_cast<float>(GetRandom(4)),
                                       pPlattForm->yPos - 15.0f, FEUERFALLE3);
            PartikelSystem.PushPartikel(pPlattForm->xPos + 33.0f + static_cast<float>(GetRandom(2)),
                                        pPlattForm->yPos + 40.0f, FLUGSACKSMOKE);
            PartikelSystem.PushPartikel(pPlattForm->xPos + 51.0f + static_cast<float>(GetRandom(2)),
                                        pPlattForm->yPos + 40.0f, FLUGSACKSMOKE2);
            PartikelSystem.PushPartikel(pPlattForm->xPos + 48.0f + static_cast<float>(GetRandom(2)),
